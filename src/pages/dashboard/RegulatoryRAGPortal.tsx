...
export const RegulatoryRAGPortal: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Page Hero Header */}
      <div className="rounded-xl border border-slate-200/80 bg-white p-5 sm:p-6 text-slate-900 flex flex-col lg:flex-row lg:items-center justify-between gap-5 shadow-xs">
        {/* Left (65–70%) */}
        <div className="space-y-1.5 lg:max-w-[68%]">
          <div className="inline-flex items-center px-2.5 py-0.5 rounded-md text-[11px] font-semibold text-slate-600 bg-slate-100 border border-slate-200 tracking-wide">
        ...
        <div className="flex flex-col sm:flex-row lg:flex-col xl:flex-row items-stretch sm:items-center lg:items-start xl:items-center gap-2.5 shrink-0">
          <button
            ...
            disabled={isCrawling}
            className="inline-flex items-center justify-center gap-2 px-3.5 py-2 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 text-xs font-semibold text-slate-700 hover:text-slate-900 transition-all disabled:opacity-50 shadow-xs"
          >
            <RefreshCw className={cn('h-3.5 w-3.5 text-blue-600', isCrawling && 'animate-spin')} />
            <span>{isCrawling ? 'Crawling Gazette Feeds...' : 'Crawl Gazette Feeds Now'}</span>
          </button>

          <div className="inline-flex items-center justify-center gap-2 px-3.5 py-2 rounded-lg border border-slate-200 bg-slate-50 text-xs text-slate-700">
            <ShieldCheck className="h-4 w-4 text-emerald-600 shrink-0" />
            <span className="font-bold text-slate-900">4</span>
            <span className="text-slate-500 text-[11px]">Official Authorities Indexed</span>
          </div>
        </div>
      </div>
      ...
      {crawledAlert && (
        <div className="p-3 bg-blue-50/80 border border-blue-200 text-blue-900 rounded-lg flex items-center justify-between text-xs font-mono shadow-xs animate-in fade-in">
          ...
          <button
            ...
            className="text-slate-500 hover:text-slate-900 font-bold px-2 transition-colors"
          >
            ✕
          </button>
        </div>
      ...
      <div className="border-b border-slate-200 flex items-center gap-2 sm:gap-6 overflow-x-auto scrollbar-none">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              {React.cloneElement(tab.icon as React.ReactElement, {
                className: cn('h-3.5 w-3.5', isActive ? 'text-blue-600' : 'text-slate-400'),
              })}
              <span>{tab.label}</span>
              {tab.id === 'versioning' && pendingRulesCount > 0 && (
                <span className="ml-1 px-1.5 py-0.2 rounded-full text-[10px] font-mono font-bold bg-amber-50 text-amber-700 border border-amber-200">
                  {pendingRulesCount}
                </span>
              )}
            </button>
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-xl border border-slate-200/80 bg-white p-5 shadow-xs space-y-4">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-50/70 border border-slate-200 text-slate-900 placeholder:text-slate-400 text-sm rounded-xl pl-10 pr-4 py-3 focus:outline-none focus:bg-white focus:ring-2 focus:ring-blue-500 shadow-xs"
              />
            </div>
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
              ...
              <div className="text-[11px] font-mono text-slate-500">
                Evaluation Date: <span className="text-blue-700 font-bold">2026-08-27 (Active Rules Only)</span>
              </div>
            </div>
          </div>

          {/* RAG Telemetry Trace Bar */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 bg-white border border-slate-200/80 rounded-xl px-4 py-2.5 text-xs font-mono text-slate-600 shadow-xs">
            <div className="flex items-center gap-2 min-w-0">
              <Cpu className="h-3.5 w-3.5 text-blue-600 shrink-0" />
              <div className="truncate">
                <span className="text-slate-400 font-medium">RAG Pipeline Status:</span>{' '}
                <span className="text-slate-800 font-semibold">Hybrid Vector Search + Knowledge Graph Active</span>{' '}
                <span className="text-blue-600 font-medium">({searchResults.matchedChunks.length} chunks retrieved)</span>
              </div>
            </div>
            <div className="flex items-center gap-1.5 shrink-0 text-slate-700">
              <span className="text-emerald-700 font-bold">{searchResults.graphTrace.nodesTraversed}</span>
              <span className="text-slate-500">Graph Nodes Traversed</span>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {searchResults.matchedChunks.length === 0 ? (
              <div className="rounded-xl border border-slate-200/80 bg-white p-8 text-center space-y-2 shadow-xs">
                <AlertTriangle className="h-8 w-8 text-amber-500 mx-auto" />
                <h3 className="text-sm font-bold text-slate-900">No Regulatory Chunks Matched</h3>
              ...
              searchResults.matchedChunks.map((chunk) => (
                <div
                  key={chunk.chunkId}
                  className="rounded-xl border border-slate-200/80 bg-white p-5 space-y-3 shadow-xs hover:border-slate-300 transition-all text-slate-900"
                >
                  {/* Header */}
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="text-[10px] font-mono font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200 uppercase">
                          {chunk.status}
                        </span>
                      </div>
                    ...
                    <div className="text-right shrink-0">
                      <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-mono font-bold text-emerald-700 bg-emerald-50 border border-emerald-200">
                  ...
                  <div className="p-3 bg-amber-50/50 border border-amber-200/80 rounded-lg text-xs text-amber-950 font-serif italic space-y-1">
      ...
      {activeTab === 'graph' && (
        <div className="space-y-4">
          <div className="rounded-xl border border-slate-200/80 bg-white overflow-hidden text-slate-900 shadow-xs">
            {/* Header: Title + Subtitle on same row on desktop, NO ICON */}
            <div className="px-5 py-3.5 border-b border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-1.5 sm:gap-4">
            ...
            <div className="p-4 sm:p-5 space-y-5">
                ...
                <div className="rounded-lg border border-slate-200 bg-slate-50/60 p-3.5 space-y-2.5">
                  ...
                  <div className="space-y-1.5">
                    {KNOWLEDGE_GRAPH_NODES.filter((n) => n.type === 'authority').map((n) => (
                      <div
                        key={n.id}
                        className="px-3 py-2 rounded-md border border-slate-200 bg-white flex items-center justify-between text-xs font-semibold text-slate-800 hover:border-slate-300 transition-colors shadow-2xs"
                      >
                        <span className="truncate pr-2">{n.label}</span>
                        <span className="font-mono text-[10px] text-blue-600 font-medium shrink-0">ISSUES →</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="rounded-lg border border-slate-200 bg-slate-50/60 p-3.5 space-y-2.5">
                  ...
                  <div className="space-y-1.5">
                    {KNOWLEDGE_GRAPH_NODES.filter((n) => n.type === 'category').map((n) => (
                      ...
                      >
                        <span className="truncate pr-2">{n.label}</span>
                        <span className="font-mono text-[10px] text-slate-500 shrink-0">APPLIES_TO →</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="rounded-lg border border-slate-200 bg-white overflow-hidden text-slate-900 shadow-2xs">
                {/* Section Header */}
                ...
                <div className="grid grid-cols-[35%_20%_45%] items-center px-4 py-2 text-[10px] font-bold text-slate-500 uppercase tracking-wider border-b border-slate-200 bg-slate-50/50">
                  <span>Source</span>
                ...
                <div className="divide-y divide-slate-100 max-h-[320px] overflow-y-auto scrollbar-thin">
                  {KNOWLEDGE_GRAPH_EDGES.map((edge, idx) => (
                    <div
                      key={idx}
                      className="grid grid-cols-[35%_20%_45%] items-center px-4 py-2.5 text-xs transition-colors hover:bg-slate-50/80"
                    >
                      {/* Source Column */}
                      <span
                        className="font-mono text-blue-700 truncate pr-3"
                        title={edge.source}
                      >
                        {edge.source}
                      </span>
                      ...
                      {/* Relationship Column */}
                      <div>
                        <span className="text-[10px] font-mono font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200 inline-block uppercase tracking-wider">
                          {edge.relationship}
                        </span>
                      </div>
                ...
                {/* Subtle Table Footer */}
                <div className="px-4 py-2 border-t border-slate-200 bg-slate-50/50 text-[11px] font-mono text-slate-500 flex items-center justify-between">
                  <span>
                    Showing {KNOWLEDGE_GRAPH_EDGES.length} of {KNOWLEDGE_GRAPH_EDGES.length} traversals
                  </span>
                  <span className="text-[10px] text-slate-400">Active Graph Mesh</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {activeTab === 'versioning' && (
        <div className="space-y-4">
          ...
          {pendingRulesCount > 0 && (
            <div className="p-4 rounded-xl border border-amber-200 bg-amber-50/60 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-slate-900 shadow-xs">
              <div>
              ...
              <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-mono font-semibold text-amber-800 bg-amber-100 border border-amber-300 shrink-0 self-start sm:self-center">
          ...
          <div className="space-y-4">
            {ruleRegistry.map((rule) => (
              <div
                key={rule.ruleId}
                className="rounded-xl border border-slate-200/80 bg-white text-slate-900 overflow-hidden shadow-xs"
              >
                {/* Rule Header */}
                <div className="p-4 sm:p-5">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    ...
                    <span className="text-xs font-mono text-slate-500">
                      Active Version: <span className="font-bold text-slate-800">v{rule.activeVersion}.0</span>
                    </span>
                  </div>
                ...
                {/* Sub-Header */}
                <div className="px-4 sm:px-5 py-2 bg-slate-50 border-t border-b border-slate-200 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                  Rule Version History &amp; Human Approvals
                </div>

                {/* Table Header (Desktop) */}
                <div className="hidden lg:grid grid-cols-[80px_130px_110px_1fr_180px_170px] items-center px-4 sm:px-5 py-2 text-[10px] font-bold text-slate-500 uppercase tracking-wider bg-slate-50/50 border-b border-slate-200">
                  <span>Version</span>
                  <span>Status</span>
                  <span>Effective</span>
                  <span>Summary</span>
                  ...
                  {rule.versions.map((ver) => (
                    <div
                      ...
                      className={cn(
                        ...
                        ver.status === 'ACTIVE'
                          ? 'border-l-2 border-l-emerald-500 bg-white hover:bg-slate-50/50'
                          : ver.status === 'PENDING_APPROVAL'
                          ? 'border-l-2 border-l-amber-500 bg-amber-50/30'
                          : 'border-l-2 border-l-slate-300 bg-white hover:bg-slate-50/50'
                      )}
                    >
                      {/* Desktop Grid Row */}
                      <div className="hidden lg:grid grid-cols-[80px_130px_110px_1fr_180px_170px] items-start gap-3">
                        {/* Version */}
                        <span className="font-mono text-xs font-bold text-slate-900 pt-0.5">
                        ...
                        {/* Effective */}
                        <span className="text-xs font-mono text-slate-600 pt-0.5">
                          {ver.effectiveFrom}
                        </span>

                        {/* Summary */}
                        <p className="text-xs text-slate-700 leading-relaxed pr-2">
                          {ver.changeSummary}
                        </p>
                          ...
                          {ver.status === 'PENDING_APPROVAL' ? (
                            <div className="flex items-center gap-1.5">
                              <button
                                ...
                                className="text-[11px] font-medium text-rose-700 hover:text-rose-800 hover:bg-rose-50 border border-rose-200 px-2 py-1 rounded transition-colors"
                              >
                                Reject
                              </button>
                              <button
                                ...
                                className="text-[11px] font-semibold text-white bg-emerald-600 hover:bg-emerald-700 px-2.5 py-1 rounded transition-colors shadow-2xs whitespace-nowrap"
                              >
                                Approve
                              </button>
                            </div>
                            ...
                            <div className="leading-tight">
                              <span className="text-slate-700 font-medium">{ver.approvedBy}</span>
                              {ver.approvedAt && (
                      ...
                      <div className="lg:hidden space-y-2 text-xs">
                        ...
                        <p className="text-slate-700 leading-relaxed">{ver.changeSummary}</p>

                        <div className="text-[11px] font-mono text-slate-500 space-y-0.5 pt-1 border-t border-slate-100">
                        ...
                        {ver.status === 'PENDING_APPROVAL' && (
                          <div className="flex items-center gap-2 pt-2">
                            <button
                              type="button"
                              onClick={() => handleRejectPendingRule(rule.ruleId, ver.versionId)}
                              className="text-xs font-medium text-rose-700 hover:text-rose-800 hover:bg-rose-50 border border-rose-200 px-3 py-1 rounded transition-colors"
                            >
                              Reject
                            </button>
                            <button
                              type="button"
                              onClick={() => handleApprovePendingRule(rule.ruleId, ver.versionId)}
                              className="text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-700 px-3 py-1 rounded transition-colors shadow-2xs"
                            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {OFFICIAL_REGULATORY_SOURCES.map((src) => {
            ...
            return (
              <div
                key={src.id}
                className="rounded-xl border border-slate-200/80 bg-white text-slate-900 p-4 sm:p-5 flex flex-col justify-between gap-4 shadow-xs hover:border-slate-300 transition-colors"
              >
                {/* Header: Abbreviation Badge + Authority Name + Description */}
                <div className="space-y-2.5">
                  <div className="flex items-start gap-2.5">
                    <span className="font-mono text-xs font-bold text-blue-700 bg-blue-50 px-2 py-0.5 rounded border border-blue-200 shrink-0 mt-0.5">
                      {abbr}
                    </span>
                ...
                <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 space-y-1.5 text-xs font-mono text-slate-700">
                  <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-2">
                    <span className="text-slate-500 font-medium sm:w-32 shrink-0">
                      Gazette Reference:
                    </span>
                    <span className="text-slate-800 truncate" title={src.gazetteRef}>
                      {src.gazetteRef}
                    </span>
                  </div>
                  <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-2">
                    ...
                    <span className="text-slate-800 truncate" title={src.documentTypes.join(', ')}>
