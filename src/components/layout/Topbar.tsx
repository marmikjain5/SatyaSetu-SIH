...
export const Topbar: React.FC<TopbarProps> = ({ onOpenCommandPalette }) => {
  ...
  return (
    <header className="sticky top-0 z-20 h-16 bg-white border-b border-slate-200/80 px-4 sm:px-6 flex items-center justify-between shadow-xs">
      ...
      <div className="flex items-center flex-1 max-w-md md:max-w-lg xl:max-w-xl min-w-0 mr-4">
        <button
          type="button"
          onClick={onOpenCommandPalette}
          className="w-full flex items-center justify-between gap-2.5 px-3.5 py-2 bg-slate-50 hover:bg-slate-100 text-slate-500 rounded-lg border border-slate-200 text-xs transition-colors group cursor-pointer shadow-2xs focus:outline-hidden focus:ring-2 focus:ring-blue-500/20"
        >
          <div className="flex items-center gap-2.5 min-w-0 flex-1">
            <Search className="h-4 w-4 text-slate-400 group-hover:text-slate-600 shrink-0" />
            <span className="text-slate-500 group-hover:text-slate-700 font-normal truncate text-xs">
              Quick Search (Products, Violations, Entities, Rules)...
            </span>
          </div>
          <kbd className="hidden sm:inline-flex items-center gap-0.5 font-mono text-[10px] bg-white border border-slate-200 px-1.5 py-0.5 rounded text-slate-500 font-semibold shadow-2xs shrink-0">
            Ctrl + K
          </kbd>
        </button>
      </div>
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <div className="hidden lg:flex items-center gap-2 px-2.5 py-1 bg-emerald-50 text-emerald-800 border border-emerald-200 rounded-md text-[11px] font-mono font-medium">
